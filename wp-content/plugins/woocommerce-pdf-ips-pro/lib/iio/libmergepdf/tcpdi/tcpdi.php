<?php
//
//  WPO_WCPDF_IPS_PRO_TCPDI - Version 1.1
...
class WPO_WCPDF_IPS_PRO_FPDF extends WPO_WCPDF_IPS_PRO_TCPDF {}

require_once('fpdf_tpl.php');

require_once('WPO_WCPDF_IPS_PRO_tcpdi_parser.php');


class WPO_WCPDF_IPS_PRO_TCPDI extends WPO_WCPDF_IPS_PRO_FPDF_TPL {
    /**
     * Actual filename
     * @var string
     *
    ...
    public function setSourceData($pdfdata) {
        ...
        if (!isset($this->parsers[$filename]))
            $this->parsers[$filename] = new WPO_WCPDF_IPS_PRO_tcpdi_parser($pdfdata, $filename);
        $this->current_parser =& $this->parsers[$filename];
        $this->setPDFVersion(max($this->getPDFVersion(), $this->current_parser->getPDFVersion()));
    ...
    protected function _getPdfParser($filename) {
        ...
        return new WPO_WCPDF_IPS_PRO_tcpdi_parser($data, $filename);
    }
    ...
    public function AddTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
        if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($page)) {
    ...
    public function _putformxobjects() {
        ...
        foreach($this->tpls AS $tplidx => $tpl) {
            ...
            $this->_newobj();
            $cN = $this->n; // WPO_WCPDF_IPS_PRO_TCPDF/Protection: rem current "n"
            ...
            $nN = $this->n; // WPO_WCPDF_IPS_PRO_TCPDF: rem new "n"
            $this->n = $cN; // WPO_WCPDF_IPS_PRO_TCPDF: reset to current "n"
            ...
            $this->_out('endobj');
            $this->n = $nN; // WPO_WCPDF_IPS_PRO_TCPDF: reset to new "n"
        }
        ...
        switch ($value[0]) {
            case PDF_TYPE_STRING:
                if ($this->encrypted) {
                    ...
                    $value[1] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($value[1]);
                }
                break;
