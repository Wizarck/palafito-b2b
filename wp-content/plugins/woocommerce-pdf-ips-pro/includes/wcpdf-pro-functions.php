...
class Functions {
	...
	public function attach_static_file( $attachments, $email_id, $order ) {
		...
		// if using Polylang get the order language (WPML has a media translation tool)
		if ( is_callable( array( WPO_WCPDF_Pro()->multilingual_full, 'get_order_lang' ) ) && class_exists( '\\Polylang' ) && did_action( 'pll_init' ) && is_callable( array( $order, 'get_id' ) ) ) {
	...
	public function address_replacements( $original_address, $document, $type ) {
		...
		$pro_address = $this->pro_settings[$type.'_address'];
	...
	public function generate_document_from_order_status( $document_type, $order_id ) {
	...
	public function multilingual_supported_plugins() {
		return apply_filters( 'wpo_wcpdf_pro_multilingual_supported_plugins', [
			'wpml' => [
	...
	public function get_active_multilingual_plugins() {
		$active_plugins = [];
