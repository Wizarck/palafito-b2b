...
class PaymentTokensMigration {
	 ...
	 */
	public function __construct( \WooCommerce\PayPalCommerce\Vaulting\PaymentTokenFactory $payment_token_factory, \WooCommerce\PayPalCommerce\Vaulting\PaymentTokenRepository $payment_token_repository, \WooCommerce\PayPalCommerce\Vaulting\PaymentTokenHelper $payment_token_helper, LoggerInterface $logger ) {
		...
		foreach ( $tokens as $token ) {
			if ( isset( $token->source()->card ) ) {
				...
				try {
					...
					$this->logger->error( "Could not save WC payment token credit card {$token->id()} for user {$id}. " . $exception->getMessage() );
					continue;
				}
			} elseif ( $token->source()->paypal ) {
				$wc_tokens = WC_Payment_Tokens::get_customer_tokens( $id, PayPalGateway::ID );
				if ( $this->payment_token_helper->token_exist( $wc_tokens, $token->id(), \WooCommerce\PayPalCommerce\Vaulting\PaymentTokenPayPal::class ) ) {
					$this->logger->info( 'Token already exist for user ' . (string) $id );
					continue;
				}
				...
				try {
					...
					$this->logger->error( "Could not save WC payment token PayPal {$token->id()} for user {$id}. " . $exception->getMessage() );
					continue;
				}
			}
		}
