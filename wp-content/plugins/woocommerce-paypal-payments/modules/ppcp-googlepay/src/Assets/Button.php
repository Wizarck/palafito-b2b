...
class Button implements ButtonInterface {
	 ...
	 */
	public function __construct( string $module_url, string $sdk_url, string $version, SessionHandler $session_handler, SubscriptionHelper $subscription_helper, Settings $settings, Environment $environment, SettingsStatus $settings_status, LoggerInterface $logger ) {
	...
	public function add_onboarding_options( $options ): string {
		...
		return $options . '<li><label><input type="checkbox" id="ppcp-onboarding-google" ' . $checked . ' data-onboarding-option="ppcp-onboarding-google"> ' . __( 'Onboard with GooglePay', 'woocommerce-paypal-payments' ) . '</label></li>';
	}
	/**
	...
	public function render(): bool {
		...
		$button_enabled_checkout = \true;
		$button_enabled_payorder = \true;
	...
	public function script_data(): array {
		...
		$shipping = array(
			...
			'configured' => wc_shipping_enabled() && wc_get_shipping_method_count( \false, \true ) > 0,
		);
		if ( $shipping['enabled'] ) {
