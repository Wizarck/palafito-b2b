...
class OrderEndpoint {
	 ...
	 */
	public function with_bn_code( string $bn_code ): \WooCommerce\PayPalCommerce\ApiClient\Endpoint\OrderEndpoint {
		$this->bn_code = $bn_code;
		return $this;
	}
	/**
	...
	public function create( array $items, string $shipping_preference, Payer $payer = null, PaymentToken $payment_token = null, string $paypal_request_id = '', string $user_action = ApplicationContext::USER_ACTION_CONTINUE, string $payment_method = '', array $request_data = array(), PaymentSource $payment_source = null ): Order {
		...
		$data   = array(
			...
			'application_context' => $this->application_context_repository->current_context( $shipping_preference, $user_action )->to_array(),
		);
		...
		if ( ! in_array( $status_code, array( 200, 201 ), \true ) ) {
			...
			$this->logger->warning( sprintf( 'Failed to create order. PayPal API response: %1$s', $error->getMessage() ) );
			throw $error;
		}
	...
	public function capture( Order $order ): Order {
		...
		if ( 201 !== $status_code ) {
			...
			$this->logger->log(
				'warning',
				sprintf( 'Failed to capture order. PayPal API response: %1$s', $error->getMessage() ),
				array(
					'args'     => $args,
					'response' => $response,
				)
	...
	public function authorize( Order $order ): Order {
		...
		if ( ! in_array( $status_code, array( 200, 201 ), \true ) ) {
			...
			$this->logger->log(
				'warning',
				sprintf( 'Failed to authorize order. PayPal API response: %1$s', $error->getMessage() ),
				array(
					'args'     => $args,
					'response' => $response,
				)
	...
	public function confirm_payment_source( string $id, array $payment_source ): stdClass {
		...
		$data     = array(
			...
			'application_context'    => array( 'locale' => 'es-MX' ),
		);
		$args     = array(
			'method'  => 'POST',
			'headers' => array(
