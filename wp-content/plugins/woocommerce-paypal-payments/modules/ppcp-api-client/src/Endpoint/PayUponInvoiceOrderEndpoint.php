...
class PayUponInvoiceOrderEndpoint {
	 ...
	 */
	public function __construct( string $host, Bearer $bearer, OrderFactory $order_factory, FraudNet $fraudnet, LoggerInterface $logger ) {
		$this->host          = $host;
		$this->bearer        = $bearer;
	...
	public function create( array $items, PaymentSource $payment_source, WC_Order $wc_order ): Order {
		$data     = array(
			...
			'payment_source'         => array( 'pay_upon_invoice' => $payment_source->to_array() ),
		);
