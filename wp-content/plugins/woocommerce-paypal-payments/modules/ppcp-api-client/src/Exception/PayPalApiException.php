 ...
 */
class PayPalApiException extends \WooCommerce\PayPalCommerce\ApiClient\Exception\RuntimeException {
	...
	public function __construct( stdClass $response = null, int $status_code = 0 ) {
		...
		if ( ! isset( $response->message ) ) {
			$response->message = sprintf(
				...
				__( 'Unknown error while connecting to PayPal. Status code: %1$d.', 'woocommerce-paypal-payments' ),
				$this->status_code
			);
		}
