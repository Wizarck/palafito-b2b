...
class PurchaseUnit {
	 ...
	 */
	public function set_amount( \WooCommerce\PayPalCommerce\ApiClient\Entity\Amount $amount ): void {
		$this->amount = $amount;
	}
	/**
	 ...
	 */
	public function set_shipping( ?\WooCommerce\PayPalCommerce\ApiClient\Entity\Shipping $shipping ): void {
		$this->shipping = $shipping;
	}
	/**
	 ...
	 * @return array
	 */
	public function to_array( bool $sanitize_output = \true, bool $allow_ditch_items = \true ): array {
		$purchase_unit = array(
			'reference_id' => $this->reference_id(),
			'amount'       => $this->amount()->to_array(),
			'description'  => $this->description(),
			'items'        => array_map(
				static function ( \WooCommerce\PayPalCommerce\ApiClient\Entity\Item $item ): array {
		...
		return $this->apply_ditch_items_mismatch_filter( $has_ditched_items_breakdown, $purchase_unit );
	}
	/**
	...
	public function apply_ditch_items_mismatch_filter( bool $ditched_items_breakdown, array $purchase_unit ): array {
		...
		if ( $ditch ) {
			...
			if ( isset( $this->sanitizer ) && $ditch !== $ditched_items_breakdown ) {
				$this->sanitizer->set_last_message( __( 'Ditch items breakdown filter. Items and breakdown ditched.', 'woocommerce-paypal-payments' ) );
			}
		}
