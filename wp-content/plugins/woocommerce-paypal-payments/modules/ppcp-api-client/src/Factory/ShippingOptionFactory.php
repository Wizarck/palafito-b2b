...
class ShippingOptionFactory {
	 ...
	 */
	public function __construct( \WooCommerce\PayPalCommerce\ApiClient\Factory\MoneyFactory $money_factory ) {
	...
	public function from_wc_cart( ?WC_Cart $cart = null ): array {
		...
		foreach ( $packages as $package ) {
			...
			foreach ( $rates as $rate ) {
				...
				$options[] = new ShippingOption( $rate->get_id(), $rate->get_label(), $rate->get_id() === $chosen_shipping_method, new Money( (float) $rate->get_cost(), get_woocommerce_currency() ), ShippingOption::TYPE_SHIPPING );
			}
		}
		if ( ! $chosen_shipping_methods && $options ) {
			$options[0]->set_selected( \true );
		}
		return $options;
	}
	/**
	...
	public function from_paypal_response( stdClass $data ): ShippingOption {
		...
		return new ShippingOption( $data->id, $data->label ?? '', isset( $data->selected ) ? (bool) $data->selected : \false, $amount, $data->type ?? ShippingOption::TYPE_SHIPPING );
	}
}
