...
class ItemFactory {
	...
	public function from_wc_cart( \WC_Cart $cart ): array {
		$items             = array_map(
			function ( array $item ): Item {
				...
				return new Item( $this->prepare_item_string( $product->get_name() ), new Money( $price, $this->currency->get() ), $quantity, $this->prepare_item_string( $product->get_description() ), null, $this->prepare_sku( $product->get_sku() ), $product->is_virtual() ? Item::DIGITAL_GOODS : Item::PHYSICAL_GOODS, $product->get_permalink(), $image[0] ?? '', 0, $cart_item_key );
			},
		...
		if ( $fees_from_session ) {
			$fees = array_map(
				function ( \stdClass $fee ): Item {
					return new Item( $fee->name, new Money( (float) $fee->amount, $this->currency->get() ), 1, '', null );
				},
				$fees_from_session
			);
		}
	...
	private function from_wc_order_fee( \WC_Order_Item_Fee $item, \WC_Order $order ): Item {
		return new Item( $this->prepare_item_string( $item->get_name() ), new Money( (float) $item->get_amount(), $order->get_currency() ), $item->get_quantity(), '', null );
	}
	/**
	...
	public function from_paypal_response( \stdClass $data ): Item {
		if ( ! isset( $data->name ) ) {
			throw new RuntimeException( __( 'No name for item given', 'woocommerce-paypal-payments' ) );
		}
		if ( ! isset( $data->quantity ) || ! is_numeric( $data->quantity ) ) {
			throw new RuntimeException( __( 'No quantity for item given', 'woocommerce-paypal-payments' ) );
		}
		if ( ! isset( $data->unit_amount->value ) || ! isset( $data->unit_amount->currency_code ) ) {
			throw new RuntimeException( __( 'No money values for item given', 'woocommerce-paypal-payments' ) );
		}
		...
		return new Item( $data->name, $unit_amount, (int) $data->quantity, $description, $tax, $sku, $category, $url, $image_url );
	}
}
