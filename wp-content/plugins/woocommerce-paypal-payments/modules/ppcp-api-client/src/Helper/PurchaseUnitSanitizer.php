...
class PurchaseUnitSanitizer {
	...
	const VALID_MODES     = array( self::MODE_DITCH, self::MODE_EXTRA_LINE );
	...
	public function __construct( string $mode = null, string $extra_line_name = null ) {
		if ( ! in_array( $mode, self::VALID_MODES, \true ) ) {
	 ...
	 * @return array
	 */
	public function sanitize( array $purchase_unit, bool $allow_ditch_items = \true ): array {
	...
	private function sanitize_item_amount_mismatch(): void {
		...
		if ( $this->mode === self::MODE_EXTRA_LINE ) {
			if ( $item_mismatch < 0 ) {
				...
				foreach ( $this->purchase_unit['items'] as $index => $item ) {
					...
					$increment = ( new \WooCommerce\PayPalCommerce\ApiClient\Helper\MoneyFormatter() )->minimum_increment( $item['unit_amount']['currency_code'] );
					// not floor items that will be negative then.
					if ( (float) $item['unit_amount']['value'] < $increment ) {
						continue;
					}
					$this->purchase_unit['items'][ $index ]['unit_amount'] = ( new Money( (float) $item['unit_amount']['value'] - $increment, $item['unit_amount']['currency_code'] ) )->to_array();
				}
			}
			$item_mismatch = $this->calculate_item_mismatch();
			if ( $item_mismatch > 0 ) {
				...
				$this->set_last_message( __( 'Item amount mismatch. Extra line added.', 'woocommerce-paypal-payments' ) );
			}
			$item_mismatch = $this->calculate_item_mismatch();
		}
		if ( $item_mismatch !== 0.0 ) {
			...
			if ( $this->allow_ditch_items && isset( $this->purchase_unit['items'] ) ) {
				...
				$this->set_last_message( __( 'Item amount mismatch. Items ditched.', 'woocommerce-paypal-payments' ) );
			}
		}
	}
	/**
	...
	private function sanitize_breakdown_mismatch(): void {
		...
		if ( $this->allow_ditch_items && $breakdown_mismatch !== 0.0 ) {
			...
			$this->has_ditched_items_breakdown = \true;
			$this->set_last_message( __( 'Breakdown mismatch. Items and breakdown ditched.', 'woocommerce-paypal-payments' ) );
		}
	}
	/**
