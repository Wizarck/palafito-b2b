...
class CachingContainer implements ContainerInterface {
	...
	public function get( $key ) {
		...
		try {
			...
			throw new NotFoundException( $this->__( 'Key "%1$s" not found in inner container', array( $key ) ), 0, $e );
		} catch ( Exception $e ) {
			throw new ContainerException( $this->__( 'Could not retrieve value for key "%1$s from inner container', array( $key ) ), 0, $e );
		}
		return $value;
	}
	/**
	...
	public function has( $key ) {
		...
		try {
			...
			throw new ContainerException( $this->__( 'Could not check cache for key "%1$s"', array( $key ) ), 0, $e );
		}
		try {
			...
			throw new ContainerException( $this->__( 'Could not check inner container for key "%1$s"', array( $key ) ), 0, $e );
		}
		return \false;
	}
	/**
