	...
	public function get( $key ) {
		...
		foreach ( $this->containers as $index => $container ) {
			...
			try {
				...
				throw new NotFoundException( $this->__( 'Failed to retrieve value for key "%1$s" from container at index "%2$s"', array( $key, $index ) ), 0, $e );
			} catch ( Exception $e ) {
				throw new ContainerException( $this->__( 'Failed check for key "%1$s" on container at index "%2$s"', array( $key, $index ) ), 0, $e );
			}
		}
		throw new NotFoundException( $this->__( 'Key "%1$s" not found in any of the containers', array( $key ) ), 0, null );
	}
	/**
	 * {@inheritDoc}
	 */
