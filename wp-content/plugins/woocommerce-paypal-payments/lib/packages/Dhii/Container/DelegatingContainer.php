...
class DelegatingContainer implements ContainerInterface {
	...
	public function get( $id ) {
		...
		if ( ! array_key_exists( $id, $services ) ) {
			throw new NotFoundException( $this->__( 'Service not found for key "%1$s"', array( $id ) ), 0, null );
		}
		$service = $services[ $id ];
		try {
			$service = $this->invokeFactory( $service );
		} catch ( UnexpectedValueException $e ) {
			throw new ContainerException( $this->__( 'Could not create service "%1$s"', array( $id ) ), 0, $e );
		}
		...
		try {
			...
			throw new ContainerException( $this->__( 'Could not extend service "%1$s"', array( $id ) ), 0, $e );
		}
		return $service;
	}
	/**
