...
class Dictionary implements IteratorAggregate, WritableMapInterface {
	...
	public function get( $key ) {
		if ( ! array_key_exists( $key, $this->data ) ) {
			throw new NotFoundException( $this->__( 'Dictionary does not have key "%1$s"', array( $key ) ), 0, null );
		}
	...
	public function withoutKeys( array $keys ): WritableContainerInterface {
		...
		foreach ( $keys as $i => $key ) {
			...
			if ( ! is_string( $key ) ) {
				throw new RangeException( $this->__( 'Key at index %1$d is not a string', array( $i ) ) );
			}
