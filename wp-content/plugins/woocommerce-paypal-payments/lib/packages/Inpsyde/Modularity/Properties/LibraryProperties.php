...
class LibraryProperties extends \WooCommerce\PayPalCommerce\Vendor\Inpsyde\Modularity\Properties\BaseProperties {
	 ...
	 * @var array
	 */
	public const EXTRA_KEYS = array( self::PROP_DOMAIN_PATH, self::PROP_NAME, self::PROP_TEXTDOMAIN, self::PROP_URI, self::PROP_VERSION, self::PROP_REQUIRES_WP );
	/**
	...
	public static function new( string $composerJsonFile, ?string $baseUrl = null ): \WooCommerce\PayPalCommerce\Vendor\Inpsyde\Modularity\Properties\LibraryProperties {
		...
		$properties[ self::PROP_TAGS ]        = $composerJsonData['keywords'] ?? array();
		$authors                              = $composerJsonData['authors'] ?? array();
		$names                                = array();
		...
		$extra = $composerJsonData['extra']['modularity'] ?? array();
	...
	private static function buildNames( array $composerJsonData ): array {
		...
		$name = mb_convert_case( str_replace( array( '-', '_', '.' ), ' ', implode( ' ', $packageNamePieces ) ), \MB_CASE_TITLE );
		return array( $basename, $name );
	}
	/**
	...
	private static function extractPhpVersion( array $composerData, string $key = 'require' ): ?string {
		...
		$base        = (array) ( $composerData[ $key ] ?? array() );
