...
if ( ! class_exists( '\\Puvox\\library_wp' ) ) {
	...
	class library_wp extends library {
		...
		public function init_module( $args = array() ) {
		...
		public function load_scripts_styles() {
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'my_styles_hook' ), 9 );
			add_action( 'admin_enqueue_scripts', array( $this, 'my_styles_hook' ), 9 );
		}
		...
		public function get_blog_name() {
			if ( is_multisite() ) {
				global $blog_id;
				$current_blog_details = ! function_exists( 'get_blog_details' ) ? get_site( $blog_id ) : get_blog_details( array( 'blog_id' => $blog_id ) );
				$b_slug               = basename( $current_blog_details->path );
				return $b_slug;
					...
					if ( get_user_option( 'rich_editing' ) == 'true' ) {
						add_filter(
							...
							function ( $plugin_array ) {
								return array_merge( $plugin_array, array( 'button_handle_' . $this->slug => $this->homeURL . '?tinymce_buttons_' . $this->slug ) );
							}
						);
						add_filter(
							'mce_buttons_2',
							function ( $button_names ) {
								return array_merge(
									$button_names,
									array_map(
										function ( $ar ) {
													return $ar['button_name'];
										},
										$this->tinymce_buttons
									)
								);
							}
						);
		...
		public function inprogress_flag_cache( $flagname, $max_seconds = 999999999 ) {
			...
			if ( $start_time && time() < $start_time + $max_seconds ) {
				...
				register_shutdown_function(
					function () use ( $flagname_final ) {
						$this->cache_set( $flagname_final, null );
					}
				);
				return false;
			}
		...
		public function inprogress_flag( $flagname, $max_seconds = 60 ) {
			...
			if ( $start_time && time() < $start_time + $max_seconds ) {
				...
				register_shutdown_function(
					function () use ( $flagname_final ) {
						$this->inprogress_flag_reset( $flagname_final );
					}
				);
				return false;
			}
		...
		public function register_stylescript( $admin_or_wp, $type, $handle = false, $url = false, $dependant = null, $version = false, $target = false ) {
			add_action(
				$admin_or_wp . '_enqueue_scripts',
				function () use ( $type, $handle, $url, $dependant, $version, $target ) {
		...
		public function get_permalink_before_init( $post = false ) {
			...
			if ( empty( $wp_rewrite ) ) {
				if ( is_object( $post ) ) {
					$link = $post->guid; } elseif ( is_numeric( $post ) ) {
					$post_obj = get_post( $post, OBJECT );
					$link     = get_permalink( $post_obj->ID ); }
			} elseif ( is_object( $post ) ) {
					$link = get_permalink( $post->ID );} else {
		...
		// NONCES
		public function default_nonce_action( $actionName = null ) {
			return ( $actionName != null ? $actionName : '_nonceAction' . '_' . $this->slug ); }
		public function default_nonce_slug( $slugName = null ) {
			return ( $slugName != null ? $slugName : '_wpnonce' . '_' . $this->slug ); }
		...
		public function submit_button( $text = null, $action = null, $slug = null, $centeredFloat = false, $echo = false ) {
			...
			if ( $centeredFloat ) {
				$out .= '<div class="centered-float submitbutton">';
			}
			$out .= get_submit_button( $text, $type = 'button-primary', $name = '', $wrap = true, $other_attributes = array( 'id' => 'mainsubmit-button' ) );
			if ( $centeredFloat ) {
				$out .= '</div>';
			}
			if ( $echo ) {
		...
		public function nonce_check( $value, $action_name ) {
			if ( ! isset( $value ) || ! wp_verify_nonce( $value, $action_name ) ) {
				die( 'not allowed. error_5151, Refresh the page' );}
		}
		...
		public function nonce_field( $name = 'nonce_input_name', $action_name = 'blabla' ) {
			return '<input type="hidden" name="' . $name . '" value="' . wp_create_nonce( $action_name ) . '" />';}


		// backend
		private function default_nonce_action_key( $actName ) {
			return 'puvox_backend_call_' . $this->slug . '_' . sanitize_key( $actName ); }
		...
		public function register_backend_call_actions() {
			add_action( 'wp_ajax_' . $this->plugin_slug_u . '_all', array( $this, 'ajax_backend_call' ) );
			add_action( 'wp_ajax_nopriv_' . $this->plugin_slug_u . '_all', array( $this, 'ajax_backend_call_nopriv' ) );
		}
		...
		public function is_plugin_active( $name ) {
			// i.e. woocommerce/woocommerce.php
		...
		public function shortcode_handler_old( $atts, $content = false ) {
			...
			if ( ! empty( $d['args'] ) ) {
				if ( ! empty( $d['args'][2] ) ) {
					...
					return call_user_func( array( $this, $name ), $args, $content );
				}
			}
		}
		...
		// ================ flash rules ================= //
		public function flush_rules_double() {
			add_action( 'wp', array( $this, 'my_flush__rewrite' ) ); }
		public function my_flush__rewrite( $RedirectFlushToo = false ) {
			...
			if ( $RedirectFlushToo ) {
				echo '<form name="mlss_frForm" method="POST" action="" style="display:none;"> <input type="text" name="mlss_FRRULES_AGAIN" value="ok" /> <input type="submit"> </form> <script type="text/javascript"> document.forms["mlss_frForm"].submit(); </script>';}
		}
		...
		public function shortcode_atts( $shortcode, $predefined_atts, $passed_atts ) {
			...
			$new_atts = shortcode_atts( $new_arr, array() );
			return $new_atts;
		}
		...
		public function sanitize_shortcode_empty_defaults( $attsArray ) {
			...
			foreach ( $attsArray as $eachAttArr ) {
				if ( empty( $eachAttArr[0] ) || in_array( $eachAttArr[0], array( '...', '___' ) ) ) {
					continue;
				}
				$new_arr[] = $eachAttArr;
			}
		...
		public function shortcode_example_string( $array, $strip_tags = false, $htmlentities = false, $ended = false ) {
			...
			$out  = '<code>';
			$out .= '[' . $array['name'] . '<span class="shortcode_atts">';
			$atts = $this->sanitize_shortcode_empty_defaults( $array['atts'] );
			foreach ( $atts  as $key => $value ) {
				$out .= ' ' . $value[0] . '="' . htmlentities( $this->truefalse_to_string( $value[1] ) ) . '"';
			} $out .= '</span>]';
		...
		public function shortcode_example( $shortcode, $array, $ended = false ) {
			...
			foreach ( $array as $key => $value ) {
				$out .= $key . '="' . $this->value_to_string( $value ) . '" ';
			}   $out = trim( $out ) . ']';
			if ( $ended ) {
				$out .= "...[/$shortcode]";
			}
	...
	<div class="shortcodes_block">
		...
		public function called_script() {
			return $_SERVER['SCRIPT_FILENAME'];}
		public function is_subscriber() {
			return $this->is_helper_( 'read' ); }
		public function is_contributor() {
			return $this->is_helper_( 'edit_posts' ); }
		public function is_author() {
			return $this->is_helper_( 'upload_files' ); }
		public function is_editor() {
			return $this->is_helper_( 'edit_others_posts' ); }
		public function is_administrator() {
			return $this->is_helper_( 'install_plugins' ); }
		private function is_helper_( $what ) {
			return ( function_exists( 'current_user_can' ) || require_once ABSPATH . 'wp-includes/pluggable.php' ) && current_user_can( $what ); }

		public function user_id() {
			return ( function_exists( 'current_user_can' ) || require_once ABSPATH . 'wp-includes/pluggable.php' ) ? get_current_user_id() : -1; }
		...
		public function post_or_cat_is_in_ansector( $upper_category_id ) {
			...
			if ( is_archive() ) {
				...
				if ( $cur_cat_id == $upper_category_id || cat_is_ancestor_of( $upper_category_id, $cur_cat_id ) ) {
					return true;}
			} elseif ( in_category( $upper_category_id, $post->ID ) ) {
					return true;
			}
		...
		public function output_table_from_db_table( $opts ) {
			$opts['excluded']     = $this->array_value( $opts, 'excluded', array() );
		...
		public function update_or_insert( $tablename, $NewArray, $WhereArray = array() ) {
			return $this->db_update_or_insert( $tablename, $NewArray, $WhereArray );
		}
		public function db_update_or_insert( $tablename, $NewArray, $WhereArray = array() ) {
			global $wpdb;
			// if string (i.e. key), then grab it from $NewArray[key]
			if ( is_string( $WhereArray ) ) {
				$WhereArray = array( $WhereArray => $NewArray[ $WhereArray ] );
			} elseif ( ! $this->array_is_associative( $WhereArray ) ) {
				foreach ( $WhereArray as $key ) {
					$WhereArray[ $key ] = $NewArray[ $key ];
				}
			}
			...
			if ( ! empty( $WhereArray ) ) {
				...
				foreach ( $WhereArray as $key => $value ) {
					$whereStr .= $wpdb->prepare( sanitize_key( $key ) . " = '%s'", $value );
			...
			if ( $wpdb->insert( $tablename, array_merge( $NewArray, $WhereArray ) ) ) {
				return true;
			}
			return false;
		}
		public function db_update_or_insert_OLD( $tablename, $NewArray, $WhereArray = array() ) {
			global $wpdb;
			// if string (i.e. key), then grab it from $NewArray[key]
			if ( is_string( $WhereArray ) ) {
				$WhereArray = array( $WhereArray => $NewArray[ $WhereArray ] );
		...
		public function db_delete_row( $tablename, $WhereArray = array() ) {
			global $wpdb;
			$whereStr = '';
			$i        = 1;
			foreach ( $WhereArray as $key => $value ) {
		...
		public function db_create_datacache_table() {
			$this->create_table_my(
				$this->db_alldata_tablename(),
				array(
		...
		public function cache_db_get( $key, $default = array(), $expire_seconds = -1, $decode = 'array' ) {
			...
			if ( empty( $res_initial ) ) {
				$res = $default;
			} elseif ( $decode && $this->maybe_json( $res_initial ) ) {
		...
		public function cache_db_append_array( $key, $data, $time = false ) {
			$existing  = $this->cache_db_get( $key, array() );
			$newData   = is_array( $data ) ? $data : array( $data );
			$finalData = array_merge_recursive( $existing, $newData );
			$res       = $this->update_or_insert(
				$this->db_alldata_tablename(),
				array(
					'option_name' => $key,
					'text'        => json_encode( $finalData ),
					'time'        => $time ?: time(),
				),
				'option_name'
			);
		...
		public function db_type_is_myisam() {
			return $this->db_type() == 'MyISAM'; }
		...
		public $notifications_excluded_db_channels = array();
		public $notifications_db_logs_maxrows      = 100;
		public function notifications_db_tablename() {
			return $GLOBALS['wpdb']->base_prefix . $this->slug . '_notificationslogs'; }
		public function notifications_db_table_create() {
			if ( ! $this->notifications_enable_db_logs ) {
				return;
			}
			global $wpdb;
		...
		public function notifications_db_entry( $id, $chat_id, $text, $time, $status ) {
			...
			if ( in_array( $chat_id, $this->notifications_excluded_db_channels ) ) {
				return;
			}

			global $wpdb;
			$this->trim_tablerows( $this->notifications_db_tablename(), $this->notifications_db_logs_maxrows );
			$res = $this->db_update_or_insert(
				$this->notifications_db_tablename(),
				array(
					'uID'     => $id,
					'chat_id' => $chat_id,
					'time'    => $time,
			...
			if ( ! $res ) {
				...
				if ( is_string( $errMsg ) && preg_match( "/Table '(.*?)" . $this->notifications_db_tablename() . "(.*?)' doesn't exist/s", $errMsg ) && ! $this->get_transient( $tr_key = $this->slug . 'tg_tablecreate_checkpoint' ) ) {
		...
		public function log( $message = '', $exception = '', $trim_chars = 1000 ) {
			...
			try {
				...
				array_shift( $trace );
				$last_func = $trace[0];// $trace=array_splice($trace, 0, 6); //get only first 6 functions
				$chain     = '';
				foreach ( $trace as $e ) {
								$chain = '[' . basename( $this->array_value( $e, 'file' ) ) . '::' . $e['function'] . '] ---> ' . $chain;}
		...
		private static $temp_tablenames_array = array();
		...
		public function show_post_categories( $vars = array(
			'POST_ID'             => false,
			'excluded_categories' => array( -1 ),
		) ) {
			$x = '';
			if ( ! $vars['POST_ID'] ) {
				$vars['POST_ID'] = $GLOBALS['post']->ID; }
			$post_categories = wp_get_post_categories( $vars['POST_ID'] );
			$cats            = array();
			foreach ( $post_categories as $c ) {
				$cats[] = get_category( $c );   }
			foreach ( $cats as $c ) {
				if ( ! in_array( $c->term_id, ( $vars['excluded_categories'] ?: array() ) ) ) {
					$x .= '<a href="' . get_term_link( $c->term_id, 'category' ) . '" target="_blank">' . $c->name . '</a>, ';
				}
			}  return $x;
		}
		...
		// ],
		// ...
		public function options_output_table( $unique_form_name = 'my_opts1', $stardard_options_array = array(), $existing_options_values = array(), $wrap_in_table = false, $wrap_in_form = false ) {
			$out           = '';
			...
			if ( $is_submission ) {
				foreach ( $stardard_options_array as $key => $block ) {
					...
					if ( in_array( $type, array( 'linebreak', 'separator' ) ) ) {
						continue;
					}
					$val = $block['default'];
					if ( is_bool( $val ) ) {
						$existing_options_values[ $key ] = isset( $_POST[ $prefix ][ $key ] );
					} elseif ( is_numeric( $val ) ) {
						$existing_options_values[ $key ] = (float) ( $_POST[ $prefix ][ $key ] );
						...
						$existing_options_values[ $key ] = $this->array_map_recursive( array( 'sanitize_textarea_field', 'stripslashes' ), $_POST[ $prefix ][ $key ] );
					}
				}
			}
			// output
			if ( $wrap_in_form ) {
				$out .= '<form class="options_form" method="post" action="">'; }
			if ( $wrap_in_table ) {
			...
			foreach ( $stardard_options_array as $key_name => $block ) {
				...
				if ( ( in_array( $type, array( 'linebreak', 'separator' ) ) ) ) {
					...
					if ( $is_bool ) {
						...
						if ( ! array_key_exists( 'disable_autoadd', $block ) ) {
							$arr[ $rand_id ] = '';
						}
			...
			if ( $wrap_in_table ) {
				$out .= '</tbody></table>'; }
			if ( $wrap_in_form ) {
				$out .= $this->submit_button( 'SAVE SETTINGS', 'nonceKeyRand_' . $prefix, 'nonceActRand_' . $prefix ) . '</form>'; }
			return array( $is_submission, $existing_options_values, $out );
		}
		...
		public function get_option_my( $keyNAME, $re_call = false, $defaultvalue = false ) {
			if ( ! isset( $this->my_custom_optioned_array ) || ! array_key_exists( $keyNAME, $this->my_custom_optioned_array ) || $re_call ) {
				...
				if ( ! array_key_exists( $keyNAME, $x ) ) {
					$x[ $keyNAME ] = false;}
				if ( $x[ $keyNAME ] == false || ! empty( $defaultvalue ) ) {
					$x[ $keyNAME ] = $defaultvalue;
		...
		public function is_posteditor_page() {
			global $pagenow;
			if ( is_admin()
			&&
			(
	...
	class wp_plugin {
		...
		public function __construct( $arg1 = array() ) {
			...
			if ( method_exists( $this, 'after_construct' ) ) {
				$this->after_construct(); // for rebasing the plugin url
			}
			$this->helpers->init_module( array( 'class' => get_called_class() ) + $arg1 );
			$this->plugin_inits();
		}
		...
		public function plugin_inits() {
			...
			if ( ! $this->helpers->above_version( '5.4' ) ) {
				register_activation_hook(
					$this->helpers->plugin_entryfile,
					function () {
						exit( __( 'Sorry, your PHP version ' . phpversion() . " is very old. We suggest changing your hosting's PHP version to latest available v7 version." ) );
					}
				);
				return;
			}
			...
			$this->plugin_files        = array_merge( ( property_exists( $this, 'plugin_files' ) ? $this->plugin_files : array() ), array( 'index.php', basename( __DIR__ ) . '.php' ) );
			...
			register_activation_hook( $this->helpers->plugin_entryfile, array( $this, 'activate' ) );
			register_deactivation_hook( $this->helpers->plugin_entryfile, array( $this, 'deactivate' ) );
			...
			add_action( 'admin_head', array( $this, 'admin_head_func' ) );
			add_action(
				'current_screen',
				function () {
			...
			if ( is_admin() && method_exists( $this->helpers, 'add_default_uninstall' ) ) {
				$this->helpers->add_default_uninstall();    // add_action( 'shutdown', [$this, 'my_shutdown_for_versioning']);
			}

			add_action( 'wp', array( $this, 'flush_checkpoint' ), 999 );
			...
			if ( $this->static_settings['has_pro_version'] ) {
				add_filter( 'upload_mimes', array( $this->helper, 'upload_mimes_filter' ), 1 );
				add_filter( 'wp_handle_upload', array( $this->helper, 'wp_handle_upload_filter' ), 10, 2 );
			}
		...
		public function my_plugin_vars( $step = 0 ) {
			...
			$temp1                        = array(); // remove sample plugin
		...
		public function activate( $network_wide ) {
			...
			if ( method_exists( $this, 'activation_funcs' ) ) {
				$this->activation_funcs( $network_wide ); }
		}
		...
		public function plugin_multi_single_site_page_select() {
			...
			} elseif ( ! $this->helper_managedfrom_NETWORK_ALLOWED() || ! $this->network_wide_active ) {
				$this->plugin_needs_network_base = false;
			}
			...
			if ( $this->static_settings['show_opts'] === true ) {
				if ( is_multisite() ) {
					add_action( 'network_admin_menu', array( $this, 'plugin__add_menu_or_submenu' ) );
				}
				add_action( 'admin_menu', array( $this, 'plugin__add_menu_or_submenu' ) );
				// redirect to settings page after activation (if not bulk activation)
				add_action(
					'activated_plugin',
					function ( $plugin ) {
						if ( $this->is_single_activation( $plugin ) ) {
										exit( wp_redirect( $this->plugin_page_url . '&isactivation' ) );
						} }
				);
			}
			...
			if ( ! array_key_exists( 'hide_plugin_links', $this->static_settings ) ) {
				...
				add_filter(
					( is_network_admin() ? 'network_admin_' : '' ) . 'plugin_action_links_' . $this->static_settings['plugin_basename'],
					function ( $links ) {
						if ( ! $this->static_settings['has_pro_version'] ) {
							$links[] = '<a href="' . $this->static_settings['donate_url'] . '">' . $this->static_settings['menu_text']['donate'] . '</a>'; }
						if ( $this->static_settings['show_opts'] ) {
							$links[] = '<a href="' . $this->plugin_page_url . '">' . $this->static_settings['menu_text']['settings'] . '</a>';  }
		...
		public function plugin__add_menu_or_submenu() {
			foreach ( $this->static_settings['menu_pages'] as $menuTitle => $menuBlock ) {
				if ( $this->helpers->array_value( $menuBlock, 'level' ) === 'mainmenu' ) {  // icons: https://goo.gl/WXAYCi
					add_menu_page( $menuBlock['title'], $menuBlock['title'], $menuBlock['required_role'], $this->slug, array( $this, 'opts_page_output_parent' ), $menuBlock['icon'] );
				} else {
					add_submenu_page( $this->settingsPHP_page_dynamic, $menuBlock['title'], '🟠 ' . $menuBlock['title'], $menuBlock['required_role'], $this->slug, array( $this, 'opts_page_output_parent' ) ); // ⭕📍🔶
				}
		...
		public function deactivate( $network_wide ) {
			if ( method_exists( $this, 'deactivation_funcs' ) ) {
				$this->deactivation_funcs( $network_wide );  }
		}
		...
		public function refresh_options() {
			$this->opts = $this->get_option_CHOSEN( $this->slug, array() );
			if ( ! is_array( $this->opts ) ) {
				$this->opts = $this->initial_user_options;
			}
				...
				if ( is_array( $block_or_value ) ) {
					if ( ! array_key_exists( $name, $this->opts ) ) {
						if ( array_key_exists( 'default', $block_or_value ) ) {
							$this->opts[ $name ] = $block_or_value['default'];
							$should_update       = true;
						}
					}
				}
		...
		public function refresh_options_time_gone() {
			...
			if ( empty( $this->opts['first_install_date'] ) ) {
				$should_update                    = true;
				$this->opts['first_install_date'] = time();
			}
			...
			if ( isset( $should_update ) ) {
				$this->update_opts(); }
			if ( isset( $reload_needed ) ) {
				$this->plugin_updated_hook( true ); }
		}
		...
		public function last_checkpoint( $var_name, $seconds_to_check = 86400 ) {
			...
			$this->$opt = ! empty( $this->$opt ) ? $this->$opt : get_option( $opt, array() );
			...
			if ( is_admin() ) {
				if ( $this->is_pro ) {
					if ( ! $this->is_pro_legal ) {
						add_action( 'network_admin_notices', array( $this, 'admin_error_notice_pro' ) );
						add_action( 'admin_notices', array( $this, 'admin_error_notice_pro' ) );
					} else {
		...
		public function reset_plugin_to_defaults() {
			...
			$this->update_phrases( array() );
			if ( method_exists( $this, 'plugin_reset_callback' ) ) {
				$this->plugin_reset_callback();
			}
		}
		...
		public function get_transient_ONCE_DELETE( $optname, $default = false ) {
			if ( ( $x = get_transient( $optname ) ) !== false ) {
		...
		public function enable_admin_debug( $exit = false ) {
			if ( WP_DEBUG ) {
				add_filter(
					...
					function ( $message, $error ) use ( $exit ) {
						return $message . $this->var_dump( $error['message'] );
						if ( $exit ) {
							die();
						}},
					10,
		...
		public function add_settings_page( $array ) {
			...
			foreach ( $actions as $act ) {
				add_action(
					$act,
					function () use ( $slug_name, $menuOpts ) {
						$menu_button_name        = $menuOpts['button_title'];
						$this->current_page_tabs = array(
							'id'      => $slug_name,
							'tablist' => $menuOpts['tabs'],
						);
		...
		public function set_sub_option( $subChildSlug, $value, $keyName = null ) {
			if ( empty( $keyName ) ) {
				$this->opts[ 'sub_' . $subChildSlug ] = $value;
			} else {
				$this->opts[ 'sub_' . $subChildSlug ][ $keyName ] = $value;
			}
			return $this->update_opts();
		}
		...
		public function helper_managedfrom_constant_value( $menuNameId = 'first' ) {
			return $this->static_settings['menu_pages'][ $menuNameId ]['default_managed']; }
		public function helper_managedfrom_constant_value_is_network( $menuNameId = 'first' ) {
			return $this->helper_managedfrom_constant_value() === 'network'; }
		public function helper_managedfrom_NETWORK_ALLOWED( $menuNameId = 'first' ) {
			return ! $this->array_value( $this->static_settings['menu_pages'][ $menuNameId ], 'network_forbidden', false ); }
		public function helper_managedfrom_SUBSITE_ALLOWED( $menuNameId = 'first' ) {
			return ! $this->array_value( $this->static_settings['menu_pages'][ $menuNameId ], 'subsite_forbidden', false ); }
		...
		public function paypal_donation_button() {
			return '<a target="_blank" href="' . $this->static_settings['donate_url'] . '" class="button" style="display:inline-block; line-height:1em; min-height:25px; color:#179bd7;"/> <img style="height:20px; vertical-align:middle;" src="' . $this->helpers->image_svg( 'paypal' ) . '" /> ' . __( 'donation' ) . '</a>'; }
		...
		public function donations_trigger_popup() {
			...
			if ( $this->static_settings['show_donation_popup'] && ! $this->helpers->array_value( $this->opts, 'donate_popup_a2' ) === '1d' ) {
				...
				if ( $this->opts['first_install_date'] != $this->opts['last_update_time'] ) {
					...
					$this->update_opts();
					$text = sprintf( __( 'Dear users, our plugin (<code>%1$s</code>) is free. However, every plugin needs noticeable amount of work by developer. If you found this plugin useful, your minimal %2$s will support developer to maintain this plugin and keep it functional. <br/>Thank you.' ), $this->static_settings['Name'], $this->paypal_donation_button() );
					?>
		...
		public function wp_kses( $text, $tagsArr = array() ) {
		...
		public function reload_without_query( $params = array(), $js_redir = true ) {
			$url = $this->helpers::remove_query_arg_esc( array_merge( $params, array( 'isactivation' ) ) );
			if ( $js_redir == 'js' ) {
				$this->js_redirect( $url ); } else {
				$this->php_redirect( $url ); }
		}
		...
		*/
		...
		public function get_phrases() {
			return $this->get_option_CHOSEN( '`translated_phrases', array() );
		}

		public function update_phrases( $array = null ) {
			if ( ! isset( $array ) ) {
				$array = $this->translation_phrases;
			}
		...
		private $current_page_tabs = array();
		public function options_tab( $tabs_array = false, $menuId = 'first' ) {
			if ( ! $tabs_array ) {
				...
				if ( ! empty( $this->current_page_tabs ) ) {
					...
					$tabs_array = array_merge( $tabs_array, $this->helpers->array_value( $this->static_settings['menu_pages'][ $menuId ], 'tabs', array() ) );
					if ( $this->helpers->notifications_enable_db_logs ) {
						$tabs_array[] = 'Notifications';
					}
					if ( property_exists( $this, 'shortcodes' ) ) {
						$tabs_array[] = 'Shortcodes';
					}
					if ( $this->define_translations_exist() ) {
						$tabs_array[] = 'Translations & Phrases';
					}
				}
		...
		public function options_output_table_full( $unique_form_name = 'my_opts1', $stardard_options_array = array(), $existing_options_values = array(), $wrap_in_table = false, $wrap_in_form = false ) {
		...
		public function settings_page_part( $type, $menuId ) {
			...
			if ( $type == 'start' ) {
				if ( $this->active_tab == 'Options' ) {
					if ( $this->helpers->extra_options_enabled ) {
						$this->custom_options_table();
					}
				}
						...
						<table class="errors_log">
							...
							<tbody>
								...
								if ( ! empty( $errors ) ) {
									...
									echo '<tr class="headerRow">'; for ( $i = 0; $i < $column_count; $i++ ) {
										$val = sanitize_text_field( $keys[ $i ] );
										echo "<th class='row_{$val}'>$val</th>";
									} echo '</tr>';
			...
			<div class="in_additional">
				...
				<ul>
					<li>
						<?php
						if ( false ) {
							printf( __( 'You can check other useful plugins at: <a href="%s">Must have free plugins for everyone</a>' ), $this->static_settings['musthave_plugins'] ) . '.';}
						?>
					</li>
				</ul>
			</div>
		...
		<div class="datachange-save-button">
			<?php submit_button( false, 'button-primary _save_button', '', true, $attrib = array( 'id' => '_save_button' ) ); ?> 
		...
		public function admin_scripts_out( $hook ) {
			// i.e. edit.php
			$where = 'admin'; // admin|wp
			if ( property_exists( $this, 'disable_admin_scripts_load' ) ) {
				return;
			}
		...
		public function admin_head_func() {
			if ( defined( 'PuvoxLibrary_scripts_loaded' ) ) {
				return;
			}  define( 'PuvoxLibrary_scripts_loaded', true );
			?>
		<script>
		...
		public function unregistered_pro() {
			return $this->static_settings['has_pro_version'] && ! $this->is_pro_legal; }
		...
		public function license_answer( $key, $type = 'check/or/activate' ) {
			$this->info_arr = array( 'key' => $key ) + array(
				'siteurl'     => home_url(),
				'plugin_slug' => $this->slug,
			) + $this->pluginvars() + $this->opts;
			$answer         = wp_remote_retrieve_body(
				wp_remote_post(
					$this->static_settings['purchase_check'] . $type,
					array(
						'method'      => 'POST',
						'timeout'     => 25,
						'redirection' => 5,
		...
		<div class="pro_block">
			...
			[data-pro-overlay=pro_overlay]::after{ 
				...
				<?php
				$str = __( 'Only available in FULL VERSION' );
				echo str_repeat( $str . '\a\a\a\a\a\a\a\a\a\a\a\a\a\a\a\a\a\a\a', 4 ) . $str;
				?>
				"; 
		...
		{ 
			...
			this_name_tt = {
				...
				show_green : function(){
					...
					alert("
					<?php
					_e( 'Thanks! License is activated for this domain.' );
					echo '\n\n\n\n';
