...
namespace WpPhpMyAdminExtension
{
	...
	class PluginClass extends \Puvox\wp_plugin {
		...
		public function __construct_my() {
			add_action( 'admin_init', array( $this, 'setup_definitions' ), 2 );
			add_action( 'admin_init', array( $this, 'if_needs_redirect_to_pma' ), 3 );
			add_action( 'wp_logout', array( $this, 'logout_pma_clear' ), 33 );
		}
		...
		public function setup_definitions() {
			...
			$this->pma_delete_dirs         = array( '/js/src', '/vendor/tecnickcom/tcpdf', '/locale', '/themes/original', '/themes/bootstrap', '/themes/metro', '/doc', '/setup', '/examples', '/install', '/js/vendor/openlayers', '/vendor/phpmyadmin/sql-parser/locale', 'js/vendor/openlayers/OpenLayers.js.map' );
			$this->pma_delete_files        = array( '/ChangeLog', '/composer.lock', '/yarn.lock' );
				...
				if ( flock( $this->locker, LOCK_EX ) ) {
					if ( ! $this->installed() ) {
						...
						foreach ( $this->pma_create_files as $eachFile ) {
							$file = $this->pma_abspath . '/' . $eachFile;
							$this->helpers->file->create_directory( dirname( $file ) );
		...
		public function create_session_file( $force = false ) {
			...
			if ( $create ) {
				$this->file_put_contents( $this->pma_sessionfile, $new_content );
			}
		}
		...
		.error_ {color:red;}
			...
			if ( $this->allow_work ) {
				...
				_e( 'WP-PHPMYADMIN can not work on your server. Try to reinstall plugin and note any issues it might show.' );
			}
			?>
			...
			<table class="form-table">
				<tr class="installed_logins">
					...
					<?php
					$error = false;
					foreach ( array_filter( $this->is_new_php ? array( 'hash', 'ctype' ) : array() ) as $extension ) {
					...
					<p class="submit automatic_login">
						...
						<a class="
						<?php
						if ( ! is_dir( $this->pma_abspath ) || $error ) {
							echo 'sample_disabled';}
						?>
						button button-primary type_auto enterb enter_automatic" target="_blank" href="<?php echo $url_to_open; ?>&automatic_login" id="installed_automatic" onclick="show_ssl_wanring1(event, this);"><?php _e( 'Enter local phpMyAdmin', 'wp-phpmyadmin-extension' ); ?></a> 
				...
				<tr class="hostinged_logins">
					...
					<td>
					<p class="submit"><a class="button button-primary type_manual enterb enter_manual" target="_blank"  href="
					<?php
					if ( empty( $this->opts['manual_pma_login_url'] ) ) {
						echo "javascript:alert('url is empty');void(0);";
					} elseif ( stripos( $this->opts['manual_pma_login_url'], '//' ) === false ) {
						echo "javascript:alert('incorrect url format');void(0);";
					} else {
						echo $url_to_open . '&hosting_pma&manual_login';
					}
					?>
					"><?php _e( "Enter hosting's phpMyAdmin", 'wp-phpmyadmin-extension' ); ?></a></p>
					<!-- <p class="submit"><a class="button button-primary type_manual enterb enter_automatic comingsoon" target="_blank"  href="
					<?php
					if ( empty( $this->opts['manual_pma_login_url'] ) ) {
						echo "javascript:alert('url is empty');void(0);";
					} elseif ( stripos( $this->opts['manual_pma_login_url'], '//' ) === false ) {
						...
						echo $url_to_open . '&hosting_pma&automatic_url';
					}
					?>
					" id="hosting_automatic" ><?php _e( 'Login Automatically', 'wp-phpmyadmin-extension' ); ?></a></p> -->
					</td>
				</tr>
				...
				<tr>
					...
					<?php
					$ip = $this->helpers->ip;
					_e( "Restrict access only to current IP (<code>$ip</code>) to login into PMA <br/>(in rare cases, if you have continiously changing dynamic IP address, then you will need to uncheck IP restriction)", 'wp-phpmyadmin-extension' );
					?>
					</td> 
		...
		private function obtainLanguagePackages() {
			...
			wp_remote_get(
				...
				array(
					'timeout'  => 300,
					'stream'   => true,
					'filename' => $this->pma_langfiles_zipPath,
				)
			);
		...
		public function file_put_contents( $path, $content, $third = null ) {
			$dir = dirname( $path );
			if ( ! is_dir( $dir ) ) {
				$this->helpers->file->create_directory( $dir );
			}
